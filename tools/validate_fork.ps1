...
Assert-Condition (([regex]::Matches($uiinit, 'nav_x_ = global\.pref\.focusmode \? \[46, 105, 174, 251, 328, 590\] : \[44, 110, 194, 291, 395, 570\]')).Count -eq 2 -and $uiinit -match 'for \( var nav_i_ = 0; nav_i_ < min\(array_length\(butt\), array_length\(nav_x_\)\); nav_i_\+\+ \) \{ butt\[nav_i_\]\.data\.x = nav_x_\[nav_i_\]; \}' -and $uiinit -match 'y_ = 20, clr_ = global\.pref\.focusmode \? ui_surfacecolor : ui_surface_high, padd_ = 8' -and $uiinit -match 'is_action:\s*true,\s*text:\s*"Export"[\s\S]{0,250}padd_multi:\s*global\.pref\.focusmode \? 5 : 8,\s*leeway:\s*global\.pref\.focusmode \? 8 : 5[\s\S]{0,300}color_butt:\s*global\.pref\.focusmode \? ui_surfacecolor : ui_accentcolor[\s\S]{0,300}color:\s*global\.pref\.focusmode \? merge_color\(ui_accentcolor, ui_textcolor, 0\.2\) : ui_bgcolor') 'Calm navigation is not compact/right-aligned, cannot reposition live, or preserves neither tab hitboxes nor legacy action colors'
Assert-Condition ($systemDraw -match 'draw_sprite_stretched_ext\(spr_pixel, 0, 0, 0, 640, 44, ui_surfacecolor, 1\)' -and $uimanager -match 'padd_multi = global\.pref\.focusmode \? 5 : 8; leeway = global\.pref\.focusmode \? 8 : 5' -and $uimanager -match 'padd_multi = 8; leeway = 5' -and $uimanager -match 'color_butt = global\.pref\.focusmode \? SYSTEMUI\.ui_surfacecolor : SYSTEMUI\.ui_accentcolor' -and $uimanager -match 'color_butt_hover = global\.pref\.focusmode \? SYSTEMUI\.ui_surface_high : merge_color\(SYSTEMUI\.ui_accentcolor, c_white, 0\.15\)' -and $uimanager -match 'color_butt = global\.pref\.focusmode \? SYSTEMUI\.ui_surfacecolor : SYSTEMUI\.ui_surface_high' -and $uimanager -match 'function nav_active_\(\)[\s\S]{0,300}color_butt = global\.pref\.focusmode \? SYSTEMUI\.ui_surfacecolor : SYSTEMUI\.ui_accentcolor[\s\S]{0,200}color = global\.pref\.focusmode \? SYSTEMUI\.ui_textcolor : SYSTEMUI\.ui_bgcolor' -and $uimanager -match 'color_butt = global\.pref\.focusmode \? SYSTEMUI\.ui_surface_high : SYSTEMUI\.ui_bordercolor' -and $uimanager -match 'function on_enter_a\(\)[\s\S]{0,250}color_butt = global\.pref\.focusmode \? SYSTEMUI\.ui_surface_high : merge_color\(SYSTEMUI\.ui_accentcolor, c_white, 0\.15\)') 'Calm/legacy navigation states, matching header fill, or tab hitboxes do not switch cleanly at runtime'
Assert-Condition (([regex]::Matches($systemDraw, 'draw_sprite_stretched_ext\(spr_pixel, 0, active_box_\.left \+ active_pad_, 40, max\(16, active_box_\.width - active_pad_ \* 2\), 2, ui_accentcolor, 1\)')).Count -eq 1 -and $systemDraw -match 'global\.pref\.focusmode && ui_tab >= 0 && ui_tab < 5 && ui_tab < count_ && !is_undefined\(butt\[ui_tab\]\.button\)') 'Calm navigation rail lacks exactly one guarded active-tab underline'
...
Assert-Condition ($luiStyles -match 'color_input_hover' -and $luiStyles -match 'color_toggle_track_active' -and $luiStyles -match 'color_scroll_thumb_active' -and $luiStyles -match 'toggle_slider_inset' -and $luiStyles -match 'text_hint_alpha' -and $luiInput -match 'self\.style\.color_input \?\?' -and $luiInput -match 'self\.style\.text_hint_alpha' -and $luiToggle -match 'self\.style\.color_toggle_thumb \?\?' -and $luiToggle -match 'self\.style\.toggle_slider_inset \?\?') 'Calm component tokens or their rendering fallbacks are missing'
Assert-Condition ($uiinit -match 'style_\.color_input = ui_surface_high; style_\.color_input_hover = ui_bordercolor' -and $uiinit -match 'style_\.color_toggle_track = ui_bordercolor; style_\.color_toggle_track_active = ui_accentcolor; style_\.color_toggle_thumb = ui_textcolor; style_\.toggle_slider_inset = 3' -and $uiinit -match 'style_\.color_scroll_track = ui_surface_high; style_\.color_scroll_thumb = ui_mutedcolor; style_\.color_scroll_thumb_active = ui_accentcolor' -and $uiinit -match 'style_\.text_hint_alpha = 1' -and $uiinit -match 'color_input:\s*global\.pref\.focusmode \? ui_surface_high' -and $uiinit -match 'color_scroll_thumb:\s*global\.pref\.focusmode \? ui_mutedcolor' -and $uiinit -match 'toggle_slider_inset:\s*global\.pref\.focusmode \? 3 : 0' -and $uiinit -match 'text_hint_alpha:\s*global\.pref\.focusmode \? 1 : 0\.5') 'Calm input, toggle, scrollbar, or readable placeholder colors are not assigned to the live and initial styles'
Assert-Condition ($uiinit -match 'style_\.setSprites\(spr_pixel, spr_pixel\)\.setSpriteCheckbox\(spr_border_header, spr_pixel\)\.setSpriteToggleSwitch\(spr_border_header, spr_pixel\)' -and $uiinit -match 'setSprites\(global\.pref\.focusmode \? spr_pixel : spr_border_undertale_outlined, global\.pref\.focusmode \? spr_pixel' -and $uiinit -match 'setSpriteCheckbox\(global\.pref\.focusmode \? spr_border_header' -and $uiinit -match 'setSpriteToggleSwitch\(global\.pref\.focusmode \? spr_border_header : spr_border_undertale_outlined, global\.pref\.focusmode \? spr_pixel') 'Calm inputs are not flat or checkbox/toggle controls have lost their structural sprites'
...
Assert-Condition ($luiScroll -match 'self\.scroll_container\.height > self\.height' -and $luiScroll -match 'var _overflow = max\(0, self\.scroll_container\.height - self\.height\)' -and $luiScroll -match '_thumb_height = max\(_scroll_width, floor\(_track_height \* min\(1, self\.height / self\.scroll_container\.height\)\)\)' -and $luiScroll -match 'color_scroll_thumb_active') 'Scrollbars are inert, fixed-size, or missing the Calm contrast tokens'
Assert-Condition ($systemCreate -match 'soupy_popup\(welcome_items_, finish_welcome_, "Continue", 420, 200' -and $systemCreate -match 'new LuiButton\(\{ text: "Open Help Guide"' -and $systemCreate -notmatch 'soupy_message\(txt_, "Open Help Guide"') 'First-run onboarding is clipped or still blocks dismissal behind the Help Guide action'
Assert-Condition ($systemDraw -match '\[c_gray\]\[scale,2\]No portrait selected' -and $systemDraw -match 'draw_sprite_ext\(spr_face_placeholder[^\r\n]+ui_mutedcolor, 0\.85\)') 'Calm empty portrait preview is still below readable contrast'
Assert-Condition ($uiinit -match 'new LuiToggleSwitch\(\{ value: bord_box_visible[^\r\n]+bindVariable\(self, "bord_box_visible"\)') 'Border visibility switch is initialized from the wrong setting'
